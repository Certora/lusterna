...
- **Each goal becomes independently inspectable** — for goals where `agrind` fails,
  use `check_lean`'s `unsolved goals` output on that line to see exactly the context and target.
...
inspect with `check_lean`'s `unsolved goals` output, pick the right tactic, and replace. This is
...
**How to detect kernel replay slowness:** if `check_lean` takes a long time on a proof even
though `trace.profiler` reports the tactics themselves as fast, the time is going into the
kernel checking the proof term — not running tactics.
...
**Keep proof terms small so builds stay fast.** If a proof is slow to check, the structure is
forcing re-elaboration of large chunks: avoid `by ...` blocks inside `apply`/`exact`/`refine`
arguments, and use `have` to create elaboration checkpoints (self-contained sub-proofs whose
results are cached).
