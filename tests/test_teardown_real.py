...
def test_shallow_clone_exports_end_to_end_with_no_manual_step(tmp_path):
    """The payment-channels scenario, end to end, with a REAL container: seed from a GENUINELY shallow
    clone (git clone --depth 1, `.git/shallow` present), run, and finalize a completed run. The export
    must succeed and the container be freed — with NO `fetch --unshallow` or any other manual step. This
    is the direct proof that a shallow clone is a first-class case, not a trap that ends in keep-alive."""
    origin = tmp_path / "origin"
    origin.mkdir()
    _git(origin, "init", "-q")
    _git(origin, "config", "user.email", "t@t")
    _git(origin, "config", "user.name", "t")
    for i in range(3):  # ≥2 commits so depth-1 genuinely leaves ancestry absent
        (origin / "src.txt").write_text(f"v{i}\n")
        _git(origin, "add", "-A")
    ...
    cid = C.start()
    try:
        C.seed_from_ref(cid, host, "HEAD", SID)  # seed straight from the shallow clone
        branch = C.run_branch(SID)
        C.exec_in(cid, ["sh", "-c", "mkdir -p verification && printf 'theorem t : True := trivial\\n' "
                                    "> verification/proof.lean"], workdir=C.REPO_IN)
        C.exec_in(cid, ["git", "add", "-A"], workdir=C.REPO_IN)
        ...
        assert outcome == "stopped"          # export was CONFIRMED — no keep-alive, no manual recovery
        assert C.is_running(cid) is False
        assert _git(host, "rev-parse", branch).stdout.strip() == tip
        assert "theorem t" in _git(host, "show", f"{branch}:verification/proof.lean").stdout
        # the review diff is exactly the run's delta over the shallow seed
        assert _git(host, "diff", "--name-only", f"{branch}-base", branch).stdout.split() \
            == ["verification/proof.lean"]
    finally:
